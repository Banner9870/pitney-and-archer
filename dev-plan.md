...
8. **Author attribution links** *(not yet implemented — pick this up at the start of any Phase 7+ session)*

   All seeded users have full profile pages reachable at `/profile/[slug]`, but author names are currently rendered as plain text throughout the app. Make them navigable:

   - **`GuideCard`** (`client/src/components/GuideCard/GuideCard.jsx`): wrap the author row (`<p className={styles.authorRow}>`) in a `<Link>` to `/profile/[author.slug]`. Stop click propagation on the link so it doesn't also trigger the card's `handleCardClick`. The `author` object is already looked up from `state.users` — just add `.slug`.
   - **`GuideDetailPage`** (`client/src/pages/GuideDetailPage/GuideDetailPage.jsx`): find the author byline in the guide header and wrap it in a `<Link to="/profile/[author.slug]">`. Same lookup pattern as GuideCard.

   **Smoke test for this task:** On the feed, click an author name on any guide card — navigates to that user's profile. On a guide detail page, click the author byline — navigates to that user's profile. Clicking the card body (outside the author name) still navigates to the guide detail page.

**Smoke test:** Navigate to `/profile/alexrivera`. Tenure badge shows "7 years in Chicago". Neighborhood history shows Pilsen → Logan Square → Lincoln Square. 4 hinge prompt cards are visible. Guides tab shows Alex's seeded guides (guide-001, guide-004, guide-009 authored by Alex). Collaborations tab shows guide-003. Saved tab is empty (no seeded saves for Alex). Navigate to `/profile/alkeefe` — newsroom header shows, no neighborhood history. Navigate to `/profile/mariam` — community member profile with their neighborhoods and 3 hinge prompts. Navigate to `/profile/yukis` — Saved tab shows guide-008 and guide-010. Navigate to a non-existent handle `/profile/nobody` — "User not found" page. Click an author name on a guide card — navigates to that author's profile. Tenure badge shows "7 years in Chicago". Neighborhood history shows Pilsen → Logan Square → Lincoln Square. 4 hinge prompt cards are visible. Guides tab shows Alex's seeded guides (guide-001, guide-004, guide-009 authored by Alex). Collaborations tab shows guide-003. Saved tab is empty (no seeded saves for Alex). Navigate to `/profile/alkeefe` — newsroom header shows, no neighborhood history. Navigate to `/profile/mariam` — community member profile with their neighborhoods and 3 hinge prompts. Navigate to `/profile/yukis` — Saved tab shows guide-008 and guide-010. Navigate to a non-existent handle `/profile/nobody` — "User not found" page.
